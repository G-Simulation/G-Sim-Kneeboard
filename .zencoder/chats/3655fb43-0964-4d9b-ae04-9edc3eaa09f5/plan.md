...
**Root Cause gefunden in `buildCoordinateArrays()` (Zeile 12849-12950):**

1. **SID (Departure)**: Zeichnet Threshold → End → SID-Punkte
   - Problem: SID sollte erst am **Ende der Runway** beginnen (wo man abhebt)
   - Aktuell: Linie beginnt am Threshold (Schwelle)

2. **STAR (Arrival)**: Zeichnet STAR-Punkte → End → Threshold
   - Problem: Man landet am **Threshold**, nicht am Ende
   - Aktuell: Linie geht erst zum Ende, dann zur Schwelle (falsch)

**Fix**: 
- SID: Nur End → SID-Punkte (nicht Threshold)
- STAR: STAR-Punkte → Threshold (nicht End)
